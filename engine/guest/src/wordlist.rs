//! The EFF SHORT WORDLIST #1 (1296 words), embedded.
//!
//! Source: <https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt>
//! ("EFF's New Wordlists for Random Passphrases", Joseph Bonneau, 2016),
//! licensed CC-BY-3.0 by the Electronic Frontier Foundation. The dice
//! numbering column of the upstream file is dropped; the words are kept
//! in the upstream (alphabetical) order, so index `i` here is the
//! upstream entry `i`.
//!
//! WHY EMBEDDED AND WHY THIS LIST (runtime/RECOVERY.md, "Derivation,
//! pinned"): the recovery phrase is GENERATED IN THE GUEST so that one
//! authority owns both the format and the derivation, and the bucket
//! object name derives from the phrase alone — which makes the exact
//! word set part of the wire contract, not a UI detail. 1296 words is
//! 2^10.34 per word, so the pinned 10 words carry ~103.4 bits.
//!
//! The words are short, unambiguous when typed, and free of prefix
//! collisions in the first three characters, which is what makes
//! transcription from screen to keyboard survivable.

/// The 1296 words, upstream order. Selection is UNIFORM over this array
/// (see `recovery::generate_phrase`, which rejects the biased tail of
/// the random range rather than taking a modulus).
pub(crate) const EFF_SHORT: [&str; 1296] = [
